...
class ImportCommand:
    def __init__(self, config_path:str = "config/common_config.yaml"):
        ...
        self.collection_dir = Path(self.config['collection_dir'])
    ...
    def __call__(self, args,):
        ...
        scene_detection_params = args.scene_detection_params
        scene_max_length = args.scene_max_length
        do_copy = args.do_copy
        do_resize = args.do_resize
        do_scenes = args.do_scenes
        do_audio = args.do_audio
        do_frames = args.do_frames
        do_thumbs = args.do_thumbs
        bulk = args.bulk
    ...
    def _import_sequential(
        ...
        with Progress(*progress_cols, transient=True) as progress:
            ...
            # detect scenes
                if do_audio:
                    subtask = progress.add_task('- Detect audio', total=None)
                    self.extract_audio(video_path, video_id, scene_detection_params, scene_max_length, force=replace, show_progress=show_progress(subtask))
                    subtasks.append(subtask)
    ...
    def _import_bulk(
        ...
        with Progress(*progress_cols, transient=True) as progress, \
            ...
            if do_audio:
                # detect scenes and extract frames
                func = lambda x: self.extract_audio(x[1], x[0], scene_detection_params, scene_max_length, force=replace)
                map_with_progress(func, video_ids_and_paths, description='Extract audios')
    ...
    def create_resized_videos(self, video_path, video_id, force=False, gpu=False, show_progress=None):
        ...
        tiny_output   = tiny_video_path
        medium_output = medium_video_path
        ...
        def ffprobe_stdout_callback(line):
        ...
        # FIXME: GPU=true error not false
        command = [
            'ffmpeg',
        ...
        def ffmpeg_stdout_callback(line):
    ...
    def detect_scenes(self, video_path, video_id, detection_params, max_length, force=False, show_progress=None):
        ...
        audio_dir = self.collection_dir / 'audio_dir' / video_id
        ...
        scenedetect_config_path = self.collection_dir / "scenedetect.cfg"
        command = [
            'scenedetect',
            '--quiet',
            '--config', str(scenedetect_config_path),
            '--input', str(video_path),
            '--output', str(selected_frames_dir),
        ] + detection_params + [
            'list-scenes',
        ...
        if max_length:
            ...
            command = [
                'python',
                'src/import/post_process_scenes.py',
                str(scene_file),
    ...
    def extract_audio(self, video_path, video_id, detection_params, max_length, force=False, show_progress=None):

        selected_frames_dir = self.collection_dir / 'selected-frames' / video_id
        audio_dir = self.collection_dir / 'audio_extracted' / video_id
        scene_file = selected_frames_dir / f'{video_id}-scenes.csv'

        # video_extract = ["ffmpeg",
        #                  " -i ./" +  str(video_path) ," -ab 160k "," -ac 2 ", " -ar 16000 ", " -vn temp.wav"]
        # ret = run_command(video_extract)
        # ffmpeg.input(video_path).output("temp.wav").run()
        command = [
            'python3',
            'src/import/extract_audio.py',
            str(scene_file),
            '--video_id',video_id,
            '--input',str(video_path),
    ...
    def create_frames_thumbnails(self, video_id, force=False, show_progress=None):
        ...
        selected_frames = selected_frames_dir / f'{video_id}-%0{n_digits}d.png'
        thumbnails = thumbnail_dir / f'{video_id}-%0{n_digits}d.jpg'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-audio', dest='do_audio', default=True, action='store_false', help='Do not extract audio.')
    ...
    import_command = ImportCommand(config_path ="config/common_config.yaml")
    import_command(args)
