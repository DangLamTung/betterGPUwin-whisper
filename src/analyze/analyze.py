...
from src.Utils.utils import read_config, is_gpu_available, run_command, progress_callback


class AnalyzeCommand:
    """ Implements the 'analyze' CLI command. """

    def __init__(self, common_config_path:str = "config/common_config.yaml"):
        self.common_config = read_config(common_config_path)
        self.collection_dir = Path(self.common_config['collection_dir'])
        self.collection_config_path = f"{self.common_config['collection_dir']}/config.yaml"
        self.config = read_config(self.collection_config_path)
        ...
        with Progress(*progress_cols, transient=True) as progress:
            ...
            for video_id in video_ids:
                ...
                for detector, params in active_object_detectors.items():
                    ...
                    self.detect_objects(detector, video_id=video_id, force=replace, gpu=gpu, params=params, stdout_callback=progress_callback(progress, subtask))
                ...
                for extractor, params in active_feature_extractors.items():
                    ...
                    self.extract_features(extractor, video_id=video_id, force=replace, gpu=gpu, params=params, stdout_callback=progress_callback(progress, subtask))
                    subtasks.append(subtask)

                # Frame clustering
                if clustering_features:
                    subtask = progress.add_task(f'- Clustering frames ({clustering_features})', total=None)
                    self.cluster_frames(video_id, features=clustering_features, force=replace, gpu=gpu, stdout_callback=progress_callback(progress, subtask))
                    subtasks.append(subtask)
    ...
    def _bulk_analyze(self, image_list_path, video_ids, n_frames, analyses, replace, gpu, **run_kws):
        ...
        with Progress(*progress_cols, transient=True) as progress:
            ...
            for detector, params in active_object_detectors.items():
                ...
                self.detect_objects(detector, image_list=image_list_path, force=replace, gpu=gpu, params=params, stdout_callback=progress_callback(progress, subtask))
                subtasks.append(subtask)
            ...
            for extractor, params in active_feature_extractors.items():
                ...
                self.extract_features(extractor, image_list=image_list_path, force=replace, gpu=gpu, params=params, stdout_callback=progress_callback(progress, subtask))
                subtasks.append(subtask)
                progress.advance(task)
    ...
    def extract_features(self, extractor, image_list=None, video_id=None, force=False, gpu=False, params={}, **run_kws):
        ...
        if extractor == 'whisper':
            input_path = self.collection_dir / 'audio_extracted' / video_id
...
if __name__ == "__main__":
    ...
    analyze_command = AnalyzeCommand(common_config_path ="config/common_config.yaml")
